...
void draw_area (cairo_t *cr, VideoBoxArea *area, gdouble r, gdouble g,
		gdouble b, gdouble a) {
  ...
  while (l != NULL) {
	...
	if (priv->pre_sel_area != NULL) {
	  ...
	  if (priv->sel_area != priv->pre_sel_area)
	    draw_area (cr, priv->pre_sel_area, 0., 0., 1., 0.6);
	}
...
void my_video_area_finalize (MyVideoArea *self) {
  MyVideoAreaPrivate *priv = my_video_area_get_instance_private (self);
  GList *key = g_hash_table_get_keys (priv->tableBox);
  GList *l = key;
  VideoBoxArea *area = NULL;
  while (l != NULL) {
    my_video_area_remove_area (self, l->data);
    l = l->next;
  }
  g_list_free (key);
...
gboolean my_video_area_pointer_motion (MyVideoArea *self, GdkEventMotion *event,
  ...
  if (priv->pressed && priv->sel_area != NULL) {
    if (priv->sel_point == FIX_NONE) {
      //move area
      my_video_area_move_area (self, priv->sel_area, event->x - priv->pressX,
			       event->y - priv->pressY);
    }
...
void my_video_area_add_area (MyVideoArea *self, gchar *name, gchar *desribe,
...
void my_video_area_rename_area (MyVideoArea *self, gchar *old_name,
				gchar *new_name) {
  MyVideoAreaPrivate *priv = my_video_area_get_instance_private (self);
  VideoBoxArea *area = g_hash_table_lookup (priv->tableBox, old_name);
  if (area != NULL) {
    g_hash_table_remove (priv->tableBox, old_name);
    g_hash_table_insert (priv->tableBox, new_name, area);
    g_free (area->label);
    area->label = g_strdup (new_name);
  }
  gtk_widget_queue_draw (self);
}
...
void my_video_area_move_area_by_name (MyVideoArea *self, gchar *name, gdouble x,
				      gdouble y) {
  MyVideoAreaPrivate *priv = my_video_area_get_instance_private (self);
  if (name == NULL) return;
  VideoBoxArea *area = g_hash_table_lookup (priv->tableBox, name);
...
void my_video_area_rotate_area_by_name (MyVideoArea *self, gchar *name,
